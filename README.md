...
- **Live widget UI** — persistent above-editor widget with animated spinners, live tool activity, token counts, and colored status icons. Configurable via `/agents → Settings → Widget`: `all` (every agent), `background` (default), or `off`
...
- **Session resume** — pick up where an agent left off, preserving full conversation context. Resumes are detached and notify you on completion, just like a fresh spawn
...
- **Model scope enforcement** — opt-in validation that subagent model choices stay within your pi `enabledModels` allowlist (sourced from `/scoped-models`, with both global and project-local pi settings honored). Caller-supplied out-of-scope → hard error to orchestrator; frontmatter-pinned out-of-scope → warning + runs anyway. Toggle via `/agents → Settings → Scope models`
...
Agents always run in the background: the call returns an ID immediately and notifies you on completion, carrying a preview of the result (use `get_subagent_result` for the full text).
...
The extension renders a persistent widget above the editor showing active agents. By default it shows detached runs (`widgetMode: background`). Switch to `all` (the same agent rows) or `off` (hide the widget) via `/agents → Settings → Widget`:
...
Agent completion notifications render Claude Code-style in the conversation:
...
Worktree isolation is configured exclusively in agent frontmatter — the `Agent` tool has no `isolation` parameter, and it stays frontmatter-only. Every other execution setting is caller-driven: an explicit `model`, `thinking`, `max_turns`, `inherit_context`, or `isolated` `Agent` tool parameter overrides the matching frontmatter default — the tool call is the orchestrator's instruction, so the caller wins whenever it supplies a value, and the agent file's frontmatter only fills fields the call leaves unspecified. All agents run detached.
...
**Background** (`maxConcurrent`, default 10). Excess top-level agents are automatically queued and start as running agents complete. The widget shows queued agents as a collapsed count. Nested children and a [workflow](#subagentworkflow)'s agents are outside this pool because their owner already accounts for their work and may be waiting on them; a workflow also bounds its own fan-out.

All subagent spawns are detached. An `Agent` or resume call returns an ID immediately, and completion notifications carry a preview; use `get_subagent_result` for the full result.
## Join Strategies
...
| Pinned in agent frontmatter (when no caller override is supplied) | Warning toast + the pinned model runs |
| Parent-inherited (neither set) | Warning toast + parent's model runs |

**Design:** `scopeModels` is a guardrail against the orchestrator picking unexpected models at runtime, not a hard policy against user-level config. A caller's explicit `model` parameter wins over the agent file's default and is refused when out of scope; when the caller omits it, a frontmatter pin still runs even when out of scope (with a visible warning).
...
**Background execution:** every `Agent` call and resume runs detached. The call returns an ID immediately, while a completion notification carries a preview and `get_subagent_result` retrieves the full result. Nested agents use the same handoff and must be collected by ID before their owner finishes.
...
Three things worth knowing about the numbers. Every token component is reported, `cacheRead` included — the cached prefix genuinely is re-read and re-billed on every call, and pi counts it the same way for the session's own messages. The extension's *display* total condenses the four components into one weighted score: `Math.round(3 * output + input + 0.2 * cacheRead + cacheWrite)`. That gives output roughly three times the weight of input, cache writes their full weight, and cache reads a reduced weight so repeated prefix reads do not dominate the reading of work done. Cost is pi's own per-message figure, priced from the model's listed rates; a model pi has no rates for contributes zero rather than an estimate. And the context-window percentage is untouched: pi derives it from assistant messages alone, so a delegating session's context doesn't appear to fill up faster. Agents that finish in the background have no tool result of their own to ride on, so their spend is carried by the next one you make — the footer catches up on the following call, not the moment they finish.

**Show cost** (`showCost`, default `false`): whether the subagent surfaces print an estimated cost beside their token counts — the widget (running *and* finished lines), [FleetView](#fleetview), the conversation viewer, results, `get_subagent_result`, and completion notifications:
...
Both places report what the run *actually* used, read back from the child session once pi has resolved its defaults and clamped the level to what the model supports — not what the call asked for. Where those differ, the request is kept beside the effective value rather than dropped: a `thinking` the caller asked for but the model couldn't reach is shown as `low (asked max)`. (An `Agent` tool parameter always overrides its frontmatter default, so a frontmatter pin can no longer displace a caller's request — the only mismatch source left is pi clamping a level down to what the model supports.)
...
| `subagents:completed` | Agent finished successfully | `id`, `type`, `description`, `status`, `durationMs`, `tokens` (display total, `{ input, output, total }` — see the note below), `usage` (the run's spend as a pi `Usage`: token components including `cacheRead`, plus `cost.total` in USD; absent when nothing was spent), `toolUses`, `result` |
| `subagents:failed` | Agent errored, stopped, or aborted | identical payload to `subagents:completed` — both are built by the same formatter, so `error` and `status` are present on that row too, just empty |
...
`tokens.total` is the weighted display score `Math.round(3 * output + input + 0.2 * cacheRead + cacheWrite)`. Upstream's `cacheRead` is the cumulative cached prefix re-read on each API call, so the reduced weight keeps it represented without allowing repeated prefix reads to dominate. Use `contextUsage.percent` (surfaced as `(NN%)` in the widget) for current context size.
...
`options` is the manager's spawn-option object, not the `Agent` tool's parameter schema. Every RPC spawn returns its id immediately, runs detached, and occupies one of the `maxConcurrent` slots (queuing when they are full); legacy `isBackground` and snake_case `run_in_background` values are ignored. It does not affect `subagents:created`, which is never emitted for an RPC spawn at all — the first event you see for one is `subagents:started`. `bypassQueue: true` starts immediately while still counting its slot. A top-level RPC spawn renders in the widget and FleetView while it runs, with the same live tool activity and turn counter an `Agent`-tool spawn gets. Nested spawns stay hidden from both.
