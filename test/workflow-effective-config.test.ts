...
describe("the workflow host reports a child's effective configuration", () => {
  ...
  // Why the workflow path never discloses a model override at all: like the
  // Agent tool, this path resolves `request.model ?? config?.model`, so the
  // script outranks the agent file and therefore always gets the model it asks
  // for. Seeding a `requestedModel` here would describe a precedence that does
  // not exist.
