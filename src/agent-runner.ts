...
export interface RunOptions {
  ...
  agentId?: string;
  /** Short task description from the Agent tool (shown in UI, used as session name). */
  description?: string;
  model?: Model<any>;
  maxTurns?: number;
  signal?: AbortSignal;
...
export async function runAgent(
  ...
  const baseSessionName = options.description ?? agentConfig?.name ?? type;
    ...
    if (event.type === "turn_end") {
      ...
      if (maxTurns != null) {
        if (!softLimitReached && turnCount >= maxTurns) {
          ...
          session.steer("This session has reached the limit, interrpt your task. Summarize the context and the work done so far so it can be hand off to a new session.");
