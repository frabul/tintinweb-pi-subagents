 ...
 *
 * Every top-level agent runs detached and is bounded by `maxConcurrent`.
 * Nested and workflow-owned children are excluded from that pool because their
 * owner already accounts for their work and may be waiting on them.
 */
 ...
 *
 * Raised from 4 when top-level spawns became detached. A limit of 4 would
 * otherwise queue the tail of the parallel fan-outs the `Agent` tool
...
/** The background pool is the only pool used by subagent spawns. */
...
interface SpawnOptions {
  ...
  /**
   * Legacy compatibility field. Every spawn is detached and runs in the
   * background; the value is ignored.
   */
  isBackground?: boolean;
  /**
   * Skip the background queue check for this spawn — start immediately even if
   * the configured concurrency limit would otherwise queue it. The slot is
   * still COUNTED once the run starts, so a bypassing spawn transiently exceeds
   * the limit rather than being invisible to it.
   *
   ...
   * of a spawn: `startAgent` can run at drain time, long after any such field
   * would have been restored, and the callback would silently never fire (or
   * fire into an unrelated caller's closure).
   */
...
interface ResumeOptions {
  /**
   * Legacy compatibility flag. Resumes started by the Agent tool are detached;
   * the value is retained for internal callers that still select the
   * synchronous workflow path.
   */
  isBackground?: boolean;
  /** Called on tool start/end with activity info (for streaming progress to UI). */
  onToolActivity?: (activity: ToolActivity) => void;
...
export class AgentManager {
  ...
  /**
   * Agents waiting to start in the background pool. Every removal path
  ...
  /** Return the background pool for top-level agents, or no pool for children. */
  ...
  spawn(
    ...
    if (pool !== undefined && !options.bypassQueue && !this.poolHasRoom(pool)) {
      // Queue it — started when a running agent in the background pool completes.
  ...
  private async startAgent(
    ...
    //
    // The pool is resolved once here and carried to `settleRun` below. The two
    // startup exits below never reach `settleRun`, so they hand the slot back
    // themselves.
    const pool = this.poolFor(record);
    const releaseSlot = () => {
   ...
   *
   * Every run fires `onComplete`; callers that synchronously consume a result
   * can pre-mark `resultConsumed` before awaiting it.
   *
   ...
   * @param pool the pool this run was charged to at start time.
   */
  ...
  /** Start queued agents up to the background concurrency limit. */
  private drainQueue() {
    for (;;) {
  ...
  /**
   * Spawn an agent and wait for completion for an internal coordinator such as
   * a workflow. The run is still recorded as a background spawn; this helper
   * only waits for its result before returning it.
  ...
  async spawnAndWait(
    ...
    // onSpawned rides on the options rather than on a field of this manager: a
    // queued spawn starts at drain time, long after any install/restore pair
    // around this call would have put the field back — and it now fires after
    // an await (worktree creation) even on the immediate path.
    ...
    const record = this.agents.get(id)!;
    // This helper's caller consumes the returned record synchronously, so its
    // completion callback must not also trigger a notification.
    ...
    // start and abort alike, so a rejection can never escape into a coordinator.
    ...
    // the immediate path owes its caller. A queued spawn's failure landed on
  ...
  async resume(
    ...
    // Detached resume: settle asynchronously and notify on completion exactly
    // like a fresh detached spawn, returning immediately with the record still
    // "running" — or "queued" when at the concurrency limit. The Agent tool
    // always selects this path; the synchronous path below remains only for
  ...
  private startResume(
    ...
    // Detached resumes from the Agent tool omit a parent signal: aborting on
    // the parent's own interrupt (user Esc) would stop work that was already
    // handed off. Internal coordinators may still pass one explicitly.
    let detachParentSignal: (() => void) | undefined;
    if (parentSignal) {
      const onParentAbort = () => this.abort(id);
  ...
  abort(id: string): boolean {
    ...
    // and no onComplete, matching the detached background contract.
